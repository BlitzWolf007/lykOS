...
override KERNEL_FILE := kernel/out/lykos-kernel.elf

override IMAGE_NAME := lykos
...
override QEMU_FLAGS := -M q35 -m 1G -d int -M smm=off -D ./log.txt -no-shutdown -no-reboot -monitor stdio $(QEMU_FLAGS_DISK)
...
.PHONY: run
run: $(IMAGE_NAME).iso
	qemu-system-x86_64 -cdrom $(IMAGE_NAME).iso -boot d $(QEMU_FLAGS)

.PHONY: run-wsl
run-wsl: $(IMAGE_NAME).iso
	qemu-system-x86_64.exe -cdrom $(IMAGE_NAME).iso -boot d $(QEMU_FLAGS)

.PHONY: run-uefi
run-uefi: ovmf $(IMAGE_NAME).iso
	qemu-system-x86_64 -bios ovmf/OVMF.fd -cdrom $(IMAGE_NAME).iso -boot d $(QEMU_FLAGS)

.PHONY: run-hdd
run-hdd: $(IMAGE_NAME).hdd
	qemu-system-x86_64 -hda $(IMAGE_NAME).hdd $(QEMU_FLAGS)

.PHONY: run-hdd-uefi
run-hdd-uefi: ovmf $(IMAGE_NAME).hdd
	qemu-system-x86_64 -bios ovmf/OVMF.fd -hda $(IMAGE_NAME).hdd $(QEMU_FLAGS)
